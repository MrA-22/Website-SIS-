                        ...
                        <div class="col-md-12 grid-margin stretch-card">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <a href="add_guru.php" class="btn btn-outline-primary btn-icon-text btn-fw" style="margin-bottom: 10px;">
                                        <i class="ti-pencil-alt btn-icon-prepend" style="margin-right: 5px;"></i> Tambah Guru
                                    </a>
                                    <div class="table-responsive">
                                        ...
                                        $query = "SELECT * FROM guru"; // Sesuaikan dengan tabel Anda
                                            ...
                                            <table class="table table-striped">
                                                ...
                                                <tbody>
                                                    ...
                                                    while ($row = mysqli_fetch_assoc($result)) {
                                                        // Pastikan bahwa atribut foto_guru memiliki data yang valid
                                                        $fotoPath = "../uploads/Foto_guru/" . (!empty($row['foto_guru']) ? $row['foto_guru'] : 'default.jpg'); // Gunakan default.jpg jika foto tidak ditemukan
                                                    ?>
                                                        <tr>
                                                            <td>
                                                                <a href="../uploads/Foto_guru/<?php echo htmlspecialchars($row['foto_guru']); ?>" target="_blank">
                                                                    <img src="<?php echo htmlspecialchars($fotoPath); ?>"
                                                                        alt="Foto Guru <?php echo htmlspecialchars($row['id_guru']); ?>"
                                                                        style="width: 75px; height: 100px; object-fit: cover; border-radius: 5px;">
                                                                </a>
                                                            </td>
                                                            <td><?php echo htmlspecialchars($row['nisn']); ?></td>
                                                            <td><?php echo htmlspecialchars($row['nama_guru']); ?></td>
                                                            <td><?php echo htmlspecialchars($row['tgll_guru']); ?></td>
                                                            <td><?php echo htmlspecialchars($row['mengajar']); ?></td>
                                                            <td><?php echo htmlspecialchars($row['alamat_guru']); ?></td>
                                                            <td><?php echo htmlspecialchars($row['nohp_guru']); ?></td>
                                                            <td>
                                                                <!-- Tombol Ubah -->
                                                                <a href="edit_guru.php?id_guru=<?php echo htmlspecialchars($row['id_guru']); ?>"
                                                                    class="btn btn-warning btn-sm">Ubah</a>
                                                                <!-- Tombol Hapus -->
                                                                <button onclick="confirmDelete(<?php echo htmlspecialchars($row['id_guru']); ?>)"
    ...
    <script>
        function confirmDelete(id) {
            if (confirm("Apakah Anda yakin ingin menghapus data ini?")) {
                window.location.href = `../php/SnG/dguru.php?id_guru=${id}`;
            }
        }
    </script>
