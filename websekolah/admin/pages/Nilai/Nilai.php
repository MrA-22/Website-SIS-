...
$sql = "SELECT id, nama_siswa, mata_pelajaran, nilai FROM rekap_nilai";
$nilai_result = $koneksi->query($sql);
...
if ($selected_kelas) {
    // Query untuk mengambil nilai siswa berdasarkan kelas
    $nilai_query = $koneksi->prepare("SELECT rn.nama_siswa, rn.mata_pelajaran, rn.nilai, k.nama_kelas 
                                    FROM rekap_nilai rn 
                                    JOIN kelas k ON rn.kelas_id = k.id_kelas
                                    WHERE rn.kelas_id = ?");
    $nilai_query->bind_param("i", $selected_kelas);
    $nilai_query->execute();
                        ...
                        <div class="col-md-12 grid-margin stretch-card">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <a href="add_nilai.php" class="btn btn-outline-primary btn-icon-text btn-fw" style="margin-bottom: 10px;">
                                        <i class="ti-pencil-alt btn-icon-prepend" style="margin-right: 5px;"></i> Tambah Nilai
                                    </a>
                                    <div class="table-responsive">
                                        ...
                                        <?php if ($selected_kelas && $nilai_result && $nilai_result->num_rows > 0): ?>
                                                ...
                                                <tbody>
                                                    ...
                                                    while ($row = $nilai_result->fetch_assoc()): ?>
                                                        <tr>
                                                            ...
                                                            <td><?= htmlspecialchars($row['mata_pelajaran'], ENT_QUOTES, 'UTF-8'); ?></td>
                                        ...
                                        <?php elseif ($selected_kelas): ?>
                                            <div class="alert alert-warning">Tidak ada data nilai untuk kelas ini.</div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </div>
        ...
        <script>
            ...
            function confirmDelete(id) {
                if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
                    ...
                    window.location.href = '../php/Nilai/nilai.php?id=' + id;
                }
            }
        </script>
        <script>
